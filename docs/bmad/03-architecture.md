...
**Decision — Postgres full-text search over the existing schemas, NOT a dedicated search cluster (ADR-037,
...
are tuned post-first-corpus — **not** a shape decision and **not** a dependency add; the ADR-037 FTS choice
...
out of scope), §8.7d (existence-hiding). *Trade recorded:* ADR-037 (Postgres FTS in-apiserver vs dedicated
...
| FR-SEARCH1…5 + NFR-PERF3 (global search, r9) | §17.5 `pkg/search` Postgres FTS over coord/scm/CRD-cache, RBAC-in-query (allowed-`project_id` predicate, existence-hiding §8.7d), injection-safe `websearch_to_tsquery`; §13 BFF proxy + top-bar surface; OQ21 relevance/index-freshness (Architecture-owned); ADR-037 |
