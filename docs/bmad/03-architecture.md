---
stepsCompleted: [context-analysis, backing-store-decision, crd-surface, agent-runtime-crd, tooling-model, coordination-record, knowledge-record, run-lifecycle, sandbox-runtime, shim-contract, credential-model, tenancy-isolation, console-frontend, sympozium-teardown, spine-risk, install-story, source-control-sync, dashboard-layer, discussion-room, build-browser, exposure-model, console-theming, plugin-architecture, event-bus, memory-backend-pluggability, adr-log, open-questions, traceability, validation, identity-rbac-auth, console-responsive]
inputDocuments:
  - ISI-2327                        # CEO cross-cutting directive 2026-08-12 (Henrik): responsive console — desktop/tablet/mobile for ALL screens — folded into §13.1/ADR-038, traceability (r26); NFR-UI added to PRD; cross-cutting to epics/testing/observability (delegated)
...
revisions:
  ...
  - r26 (2026-08-12, ISI-2188 ← Story 1.2 CRD-type definition / CEO Epic 5.7 dep): **closed the §5.1 `Agent` CRD spec gap flagged by Epic 5.7** — the `Agent` row now explicitly carries **`modelEndpointRef?`** (§10.3 BYO model-endpoint seam, ADR-026 — the field Epic 5.7 declared "arch §5.1 now adds… Epic 1 story 1.2 must add that field at Gate 2 before 5.7 builds") plus **`roleRef` (→`Role`)** and **`skillRefs[]` (→`Skill`)** (the Agent's behavior-profile + granted-skill references the Story 1.2 AC names — previously implicit via `Role.defaultSkills`), and clarified **`capabilityOverrides` = the agent-card capability overrides** (the AC's `agentCardOverrides`). **No new mechanism, no new CRD** — this is a spec-completeness reconciliation so the six squad-composition CRD types (Story 1.2 / ISI-2188) are fully specified for `api/v1alpha1` kubebuilder scaffolding; `modelEndpointRef` rides the existing §10.3 seam (Secret-ref endpoint + per-`Agent` `model`, `byoModelEndpoint` capability). `Run.spec.workItemRef` stays an **opaque coordination-DB pointer, not an owned etcd object** (ADR-001, §5.1 note). Story file: `docs/bmad/stories/1-2-define-crd-types.md`. Touchpoints §5.1 (Agent row); no locked decision reopened (ADR-001 one-Postgres, ADR-026 model seam, §5.3 runtime/model axis split all intact).
  - r27 (2026-08-12, ISI-2340 ← Story 2.1 review ISI-2339 F1, resolve-before-8.11): **coord audit table shape ruling — audit-only, not a trace firehose (ADR-040, §6.5).** The Story-2.1 migration unified the §6.5 coordination audit and the §10.1 shim Run-trace firehose into one `run_event` table; the review (F1) flagged that the append-only trigger makes the high-volume firehose **unprunable** (retention would force partition-DROP, which *bypasses* the "structurally immutable" claim) and interleaves trace noise into the "monotonic audit sequence" (gaps under an `event_type` filter). **Ruling (Architect authority, Arch §6.1):** the Postgres coord audit table (`coord.audit_log`, née `run_event`) carries **ONLY the low-volume immutable coordination audit** (claim/comment/artifact/state-transition/completion + principal + `initiated_by_user_id` + fence + timestamp, clean monotonic `bigserial`); the **high-volume shim trace** (`tool_call|llm_call|build_output|error`) does **NOT** persist to Postgres in v1 — it rides **SSE live (§10.1) + opt-in OTel (§17.2)**, the "Run logs = coord audit + SSE, no new data path" model **already pinned in §13**. **Story 8.11 wires the shim trace to SSE + OTel emission, not a coord table (no 8.11 migration).** Chose this over the review's two offered options — a time-partitioned Postgres `run_trace` table (a new stateful surface re-storing telemetry OTel already owns; the working-tree migration draft's over-reach, corrected) and single-table declarative partitioning + DROP-PARTITION retention (partition-DROP bypasses immutability + trace-noise `bigserial` gaps — the F1 defect). Smallest design: **deletes** a would-be table rather than adding partition machinery, keeps the audit honestly immutable, simplifies 8.11. Lockstep remediation on `K8squad` PR #5 (`0001_coord_schema.sql` run_event→audit_log audit-only + F2 TRUNCATE guard + F6/F7 cross-ref/residual notes; `0001_coord_schema_test.sql` +§6.2-acquire assertion F3; `ci.yml` +postgres-service self-check job F4). Touchpoints §6.5, §18 ADR-040; no locked decision reopened (ADR-001 one-Postgres, §6 coordination spine, no-P2P all intact).
  - r28 (2026-08-13, ISI-2343 ← Story 1.2 code-review follow-up of ISI-2188): **reconciled the §5.1 `Run` row to Story 1.2 AC6**, mirroring the r26 `Agent`-row treatment so the single type "everything downstream watches" reads consistently whether a dev encodes the §5.1 table verbatim or reads AC6. Three changes, **no new mechanism, no new CRD, no locked decision touched**: (1) **renamed `workItemSelector` → `workItemRef`** in both the Run row and the "not CRDs" note (AC6 + the ISI-2188 ticket carry `workItemRef`; the story self-discloses they are the same field — `workItemRef` is the arch table's `workItemSelector` under the ticket's name — but the doc still carried the old name; kept it an **opaque coordination-DB pointer, not an owned etcd object**, ADR-001); (2) **added the two AC6 spec fields absent from the row** — `inputs` (free-form run params → §8.5 context envelope) and `sandboxPolicy` (RuntimeClass/isolation input → §9.1), resolving Story Open Question #1 (both are Run-spec fields, modeled opaque/structured, refined in Epic 1.3 reconcilers — not resolved elsewhere); (3) **added the `status` subresource** the row lacked — `phase` (§8 enum `Pending|Claiming|Running|Paused|Succeeded|Failed|Cancelled`, CEL-validated), `sandboxRef`, `claimedAt`, `conditions`, `artifactRefs` (refs to §6.1 artifact rows, not embedded). Spec-completeness only: lands at Gate 2 for §5.1 consistency **before Epic 1.3 reconcilers resolve these fields**; non-blocking for authoring the six types (AC6 was already internally complete). Story file: `docs/bmad/stories/1-2-define-crd-types.md` (AC6 + Open Question #1). Touchpoints §5.1 (Run row + "not CRDs" note); no locked decision reopened (ADR-001 one-Postgres / DB-pointer, ADR-002 desired-state-vs-coordination split, §8 Run lifecycle enum all intact).
...
| `Agent` | One agent instance in a squad | `runtimeRef` (→`AgentRuntime`), `roleRef` (→`Role`), `skillRefs[]` (→`Skill`), `credentialSecretRef`, `capabilityOverrides` (= agent-card capability overrides), `model`, `modelEndpointRef?` (§10.3 BYO model endpoint — ISI-2188 / Epic 5.7 dep, CEO 2026-08-11), `contextBudgetOverride?` (§8.5), `fallbackModel?` (§8/§10.3 rate-limit fallback) | Agent reconciler → validates Secret + runtime, publishes Agent Card |
...
| `Run` | Unit of squad work | **spec:** `teamRef`, `projectRef`, `workItemRef` (opaque coordination-DB pointer, ADR-001 — the arch table's prior `workItemSelector`; **renamed r28** to the Story 1.2 AC6 / ISI-2188 field name — the Run *references* the work item, it does not own/embed it), `inputs` (free-form run params folded into the §8.5 context envelope), `sandboxPolicy` (RuntimeClass/isolation selection input to §9.1 sandbox assembly), `agents[]`, `retryPolicy`; **status subresource:** `phase` (§8 enum `Pending\|Claiming\|Running\|Paused\|Succeeded\|Failed\|Cancelled`, CEL-validated), `sandboxRef`, `claimedAt`, `conditions`, `artifactRefs` (refs to artifact rows §6.1 — **not embedded**) | **Run reconciler (the core state machine, §8)** |
...
> `workItemRef` (r28, née `workItemSelector` — Story 1.2 AC6 / ISI-2188 field name); it does not embed them.
...
**Audit table shape — audit-only, NOT a trace firehose (F1, ISI-2339 → ISI-2340; ADR-040).** The
durable audit rows live in a dedicated append-only table (`coord.audit_log`, née the Story-2.1
`run_event`): one clean **monotonic `bigserial`** sequence carrying **only the low-volume coordination
events** — claim acquired/renewed/released, comment added, artifact registered, state transition,
completion — each with `principal` + `initiated_by_user_id` (§12.4) + fence + timestamp. It is
append-only-by-trigger (UPDATE/DELETE/**TRUNCATE** all rejected) and therefore **structurally immutable
without qualification**. The **high-volume shim Run-trace firehose** (`tool_call | llm_call |
build_output | error`, §10.1) does **NOT** persist to this table — nor to any new Postgres table in
v1. It rides **SSE live (§10.1, ephemeral)** + **opt-in OTel export (§17.2)**, exactly the "Run logs =
coord audit (§6.5) + SSE, **no new data path**" model already pinned in §13. **Story 8.11 wires the
shim trace to SSE + OTel emission, not to a `coord` table.** Splitting the two by *volume and retention
semantics* is what keeps the audit both immutable and prunable-free (it is never retention-managed —
coordination-event volume is bounded), while the firehose stays prunable because it never lands in the
source-of-truth store. This resolves the F1 unification defect without adding a stateful surface or
softening the immutability claim.
...
### 11.2 Seat-model & AUP boundary — one OAuth token = one human seat (ISI-2290, ratified CEO 2026-08-12)

> **The §11.1 "one login, many mounters" pattern is scoped to a *single human seat's own work* — it is
> not a licence to pool one subscription across different people or to power a hosted product.** Ratified
> from `docs/bmad/research/isi2290-anthropic-aup-byo-seat-posture.md` (v3), confirmation accepted
> 2026-08-12. The memo's finding is binding: pooling agents on one company Pro/Max subscription is **not
> AUP-defensible** (Anthropic Consumer Terms §2 no-sharing, §3 no-automated-access-except-API). Henrik's
> Paperclip finding proves single-credential concurrency *works technically*; ISI-2290 constrains *when it
> is licensed*.

Two credential **classes**, chosen by *who the consumer is*, not by runtime family:

| Consumer | Compliant credential | Rationale |
|----------|---------------------|-----------|
| **A human running their own squad** (self-hosted operator, their own Pro/Max/Team seat) | **BYO-per-human-seat** OAuth — one `CLAUDE_CODE_OAUTH_TOKEN` per human's *own* subscription, mounted by that seat's pods (§11.1). Multiple pods may share it **only while serving that same human's own work**. | Consumer Terms permit personal-tool use incl. Claude Code automation; carries a residual commercial-use risk needing a named owner's acceptance. |
| **A shared / hosted / multi-agent / product backend** serving *other* people | **Service account** — Workload Identity Federation (keyless cloud identity) **or** an org **Anthropic API key under Commercial Terms**. **Never** a human-seat consumer token fanned across replicas serving different people. | Commercial Terms explicitly permit powering a product; API keys are metered per-token with no shared-account clause. |

**Hard rules (encode in the credential stories):**
- **Do NOT** fan one subscription/seat token across replicas serving *different* people, and **do NOT**
  embed a human-seat OAuth token into a shared platform backend. The §11.1 shared-Secret mount is
  per-seat, single-owner — the credential controller's leader-election is *per human Secret*, not a
  fleet-wide token bus.
- **Enterprises are NOT API-only.** A negotiated Enterprise agreement supports **both** paths at once —
  per-seat OAuth for humans doing their own work **and** service-account/WIF/API for machine/hosted
  serving. Do not model the enterprise path as "API key or nothing." (Negotiated Enterprise contracts may
  carry custom terms that further widen this — the memo is non-binding on a specific signed agreement.)
- **Canary + rotation stay on the human-seat path.** The §11.1 controller keeps a **credential health
  canary / liveness probe** (a cheap auth check that flips the Secret to `expired` *before* a Run hits it)
  plus the documented **rotation runbook** (the manual re-login / seat-swap procedure carried over from the
  retired ISI-2112 spike) so a lapsed seat is a legible `Paused(cred_expired)`, never an opaque failure.

*Satisfies:* the ISI-2290 ratified constraint (AC gates on Epics 3/4/5/7). *Trade recorded:* ADR-041.
*Touchpoints:* §11.1 (per-seat lifecycle), §5.2 (controller is per-Secret), §10.1 (shim credential-class
metadata), Epic 7 (7.1/7.7 seat scope + new 7.8 machine path), Epic 5 (5.4 injection carries credential
class), Epic 3 (2.10 re-route respects the seat boundary).
...
*Satisfies:* FR-F1…F7, NFR-USE1/USE2, NFR-UI (responsive, §13.1), S2/S3; Theme I (FR-I1…I8), Theme J (FR-J1…J4), Theme K
(FR-K1/K2). *Trade recorded:* ADR-013 (Next.js BFF vs
SPA-direct-to-kube), ADR-020 (consumption model), ADR-021 (build-browser read model), ADR-038 (responsive: CSS-first single tree). *Depends on:*
Graphic Designer UX direction (parallel, PRD §11.4) — theme tokens for both modes.

### 13.1 Responsive Layout — desktop / tablet / mobile (Theme F, NFR-USE2 / NFR-UI; ISI-2327, CEO 2026-08-12)

**Decision (ADR-038): CSS-first, single responsive tree — one app, no separate mobile build, no device-class routing.** The console is responsive across **desktop, tablet, and mobile at v1** (a CEO cross-cutting requirement, not deferred). Layout keys off **viewport / container width**, never user-agent sniffing (unreliable, cache-hostile, and it splits the render tree). The Next.js server renders **one** SSR tree; the browser reflows via **Tailwind breakpoint utilities + CSS container queries** on the token system already chosen (§13). **Ponytail: reuse Tailwind's responsive engine — no new UI framework, no device-detection library.**

- **Canonical breakpoints (design tokens, shared with the UX kit and the test matrix):**
  - **Mobile `< 768px`** — Tailwind `base…sm`. Top bar + **5-item bottom nav** + **slide-in drawer** for overflow. All screens work in **portrait down to 360px**.
  - **Tablet `768–1024px`** — `md`. **Collapsible icon rail** (tap to expand as an overlay — no layout shift, no bottom nav).
  - **Desktop `> 1024px`** — `lg+`. **Full left rail with labels**, no bottom nav.
- **Navigation is the single adaptive shell.** Desktop/tablet/mobile are **three breakpoint expressions of the ONE adaptive-nav model already specified** (role-based nav + screens 16/18). Breakpoint collapse and RBAC filtering **compose**: a non-admin on mobile sees the **role-filtered** surface fit into the bottom-nav budget, overflow spilling to the drawer — the authorized-only selector rule (§12.3) is unchanged and there is **no responsive-specific authz path**.
- **Per-screen responsive behavior** (reflow of the *same* screen — never a reduced feature set):
  - **Kanban:** horizontal scroll on mobile; columns collapse to **swipeable tabs**; the sub-ticket tree (ISI-2322) **expands inline** within the card, as on desktop.
  - **List:** same table, **fewer columns on mobile (ID + Title + Status)**; a row **expands** for the rest. Sort/filter params (server-side, §13) are viewport-independent.
  - **Project Dashboard:** cards **stack vertically**; the activity feed moves **below** the tiles. Each tile still degrades independently (ADR-020).
  - **File / build browser:** the file tree **collapses to a dropdown** on mobile; the code/diff view goes **full-width**. Read-only still (R6).
  - **Agents org views (Team org, Role swimlanes, Leadership tree):** **pinch-to-zoom + horizontal scroll** for the tree; **role swimlanes stack vertically** on mobile. Read model, no mutate affordance (§13).
  - **Global search (§17.5):** **full-width** on mobile, **icon-triggered** on tablet, inline on desktop.
  - **Login:** centered card, works at **360px**.
  - **Settings tree:** **flattens to an accordion** on mobile.
- **Touch parity is a v1 acceptance bar, not polish:**
  - **Touch targets ≥ 44px** (WCAG 2.5.5 / platform HIG).
  - **No hover-dependent UI** — every hover affordance (org-diagram tooltips, kanban card actions, table row menus) has a **tap/focus equivalent**; a hover-only reveal is a defect, not a degradation.
  - **Pull-to-refresh** on list/feed views; **pinch-to-zoom** on diagrams (org charts, build diffs).
- **Data path is untouched.** Responsive is **pure presentation** — identical BFF payloads, the same deny-by-default RBAC wall (§12.3), the same single SSE `EventSource` bus (§13). There is **no** viewport-conditioned API, and SSR sends the same tree to every device (the browser, not the server, decides layout). This keeps the one authorization choke point intact.
- **Downstream lockstep (cross-refs so the spec set stays coherent):** the **viewport test matrix** (360 / 768 / 1024 / 1440px × iOS Safari / Android Chrome / desktop Chrome+Firefox) is a v1 gate in **05-testing-strategy**; a **viewport / device-class dimension** rides the existing OTel console RUM (§17.2) in **04-observability-plan** so real breakpoint usage is observable; **02-prd** carries the requirement as **NFR-UI**; **04-epics-and-stories** attaches a mobile+tablet AC to each Epic 8 console screen.
- **Scope guard (R6, restated for responsive):** responsive = **reflow + touch parity of the same screens at every breakpoint** — **not** a "mobile-lite" reduced product and **not** a native app. Every screen is fully functional at every breakpoint.

*Satisfies:* NFR-UI (new, PRD Theme F), NFR-USE2; ISI-2327. *Trade:* ADR-038. *Depends on:* Graphic Designer breakpoint/spacing tokens (parallel, PRD §11.4).
...
### 17.5 Global Cross-Entity Search (Theme P, FR-SEARCH1…5; ISI-2323 — CEO-validated mocks 2026-08-12)

**What it is.** An always-visible top-bar search that finds **work items/tickets, Runs, files/artifacts,
agents, and Projects** from one query (PRD FR-SEARCH1). It is a **derived read-model** — it introduces
**no new store of record**, no coordination path, and **no new authority**: it can only surface what the
caller could already reach (S2 legibility, R6 scope guard). The two-records invariant (§4, ADR-001) is
untouched.

**Decision — Postgres full-text search over the existing schemas, NOT a dedicated search cluster (ADR-039,
ponytail rung 3: the stdlib/engine already does this).** v1 search rides **PostgreSQL FTS** (`tsvector` /
`tsquery` + a **GIN** index, `pg_trgm` for prefix/typo-tolerant name matching) over the tables KSquad
already owns. *Rationale:* ADR-001 — "S1 (≤4h install) punishes every added stateful dependency"; a
dedicated index (OpenSearch/Elasticsearch) would be a **third** operational surface, a **dual-write/reindex
lag** to manage, and a new blast-radius/tenancy surface. Postgres FTS answers the v1 corpus (control-plane
metadata, not a document lake) with **no new dependency and no reindex pipeline** — records are searchable
**at query time, live**. *`ponytail:` a dedicated index is the **named upgrade path*** when corpus size or
relevance quality outgrows FTS (the OQ21 trigger) — we do **not** build it until a measured need appears.

**Placement — `pkg/search` inside the apiserver, exposed on the apiserver, proxied by the BFF (§13).** Search
is a **library package in the existing apiserver**, not a new deployment (ponytail rung 2 — reuse the trusted
control-plane service that *already* holds the `coord`/`scm`/`auth` schemas and the deny-by-default RBAC
middleware, §12.3). One `GET /api/v1/search?q=&projectId=&types=` endpoint; the Next.js BFF proxies it under
the **same identity-aware choke point** as every other read (§13 — browser never touches Postgres/kube). No
new signing key, no extra network hop, no fourth Go binary.

**Sources & what is (not) searched.**
- **Record-backed** (Postgres FTS, live): `coord.work_item` (title/body), `coord.comment` (body),
  `coord.artifact` + **build-snapshot `meta`** (artifact kind + **file paths/filenames**, §8.7c — *paths, not
  file contents*), `scm` mirror (synced issue/PR titles, §5.4), `Run` metadata via the coord/run projection.
- **CRD-backed** (`Agent`/`Team`/`Role`/`Project`): these live in **etcd, not Postgres** — searched via the
  apiserver's **informer/lister cache** (name + labels + spec summary), near-real-time, no projection table.
  Low cardinality, so a cache scan + `pg_trgm`-style match in-process is sufficient for v1.
- **Explicitly NOT searched in v1:** **memory-record *content*** (FR-E7 trust boundary — stored knowledge is
  untrusted, provenance-gated input, not a trusted search corpus; only surfaced through the memory MCP path,
  §7.3) and **raw file/blob bodies** (build browser stays the read surface for contents, §13). These are
  Phase-2 items behind the same trust rules, not v1 scope.

**RBAC scoping is applied *in the query*, never by post-filtering (FR-SEARCH3, D9, NFR-SEC10 — the
load-bearing rule).** On every request `pkg/search` resolves the caller → `auth.project_memberships`
(§12.3) → the **allowed `project_id` set**, and injects `WHERE project_id = ANY($allowed)` (plus the
`global_role=admin` fleet-wide bypass) into **every** sub-query **before** ranking or `LIMIT`. Consequences:
an out-of-scope entity **never enters** the result set, previews, **or result counts** — the same
**existence-hiding** rule as the build browser (§8.7d: out-of-scope → absent, not `403`). CRD-backed results
are filtered by the same allowed-`Project` set (and `Team`-scope for agents, §12.1) against the informer
cache. Search is thus **just another read through the §12.3 wall** — it opens no side door around it. The
`viewer`/`contributor`/`maintainer` grade doesn't change *visibility* (all three can read within a Project),
so search scopes on **membership**, not grade.

**Query construction is injection-safe by construction (FR-SEARCH5, NFR-SEC10).** User input is **never**
concatenated into SQL or a `tsquery`. Free text is turned into a query with **`websearch_to_tsquery`** (or
`plainto_tsquery`) — which treats input as **literal terms** and tolerates arbitrary `:`, `*`, quotes, path
separators, and FTS operators **without** letting them reach the engine as executable syntax; all binds are
**parameterized**. So special/reserved characters are literal search text (never an error, never operator
injection), an **empty query** short-circuits to a neutral/recent state server-side (no wildcard table scan),
and a **no-match** returns an empty grouped result (not an error) — the API contract the console renders as
FR-SEARCH5's empty states.

**Ranking & response.** Each source returns candidates scored by `ts_rank` (FTS) / trigram similarity
(names), combined with a **recency boost** and a small **entity-type weight**, normalized so cross-source
results merge into one ranked list, then **grouped by entity type** for the dropdown (FR-SEARCH2). The API is
**stateless**; **debounce + keyboard nav + the grouped dropdown are console concerns** (§13, story 8.x). The
`projectId`/`types[]` params (FR-SEARCH4) **narrow within** the RBAC floor — they can only shrink, never
widen, the authorized set. Performance rides NFR-PERF3: the GIN index keeps interactive latency low, and the
RBAC predicate bounds every scan; search load is off the correctness-critical claim/reconcile path.

**Open (OQ21 — Architecture-owned, relevance/freshness tuning).** Exact relevance weighting (field boosts,
recency decay), CRD-cache match strategy at larger fleet sizes, and the numeric interactive-latency target
are tuned post-first-corpus — **not** a shape decision and **not** a dependency add; the ADR-039 FTS choice
holds until a measured miss triggers the dedicated-index upgrade path.

*Satisfies:* FR-SEARCH1…5, NFR-PERF3; ties FR-AUTH3/NFR-SEC10 (RBAC-in-query), FR-E7/§7.3 (memory content
out of scope), §8.7d (existence-hiding). *Trade recorded:* ADR-039 (Postgres FTS in-apiserver vs dedicated
search cluster; dedicated index = named upgrade path). *Touchpoints:* §4/ADR-001 (one Postgres), §5.4 (scm
source), §6.1 (coord source), §8.7c (build-snapshot paths), §12.3 (RBAC choke point), §13 (BFF proxy +
console surface), §17.3 (layout — `pkg/search`).
...
| 020 | Consumption attribution & metering provenance + dashboard aggregation (Theme I, FR-I2/I3/I6/I7, NFR-OBS3; ISI-2146/2325) | **Axes anchored to Run lifecycle (operator) + kubelet/cAdvisor — non-forgeable; runtime-reported token counts are best-effort, sanity-bounded, not the billing axis (OQ14); OTel metrics labeled per-principal; estimate via price table; token-consumption *trend* = time-series query over the same metrics seam; no billing DB. Dashboard is a **BFF read model** that composes coord/scm/metrics/Run-state — no aggregation service, no rollup DB; each tile degrades independently (r24)** | Trust agent self-reported tokens as authoritative (forgeable — NFR-OBS3 forbids); dedicated usage/billing datastore (new stateful dep, breaks S1); read provider billing API (BYO — no shared billing visibility); a standalone dashboard aggregation microservice or materialized rollup DB (extra tier + stale-data surface for data that already lives in coord/scm/metrics) |
...
| 037 | Issues dual-view — board-state derivation + human DnD status transition (FR-B5/FR-F1; CEO mock validation 2026-08-12, ISI-2326) | **Kanban columns = the canonical ordered `work_item.state` enum {backlog, todo, in_progress, in_review, done}; the board is a **pure projection of `state`, not a second stored "column" field** (single source of truth, no derived-state drift). `blocked` is refined to an **orthogonal condition** (flag + reason), not a workflow state — a blocked item keeps its `state` and shows a badge → five columns, not six (§8.6 wording refined). **DnD moves a card = a human status transition** via `PATCH …/work-items/{id}/state {to, expectedFrom}` through the apiserver BFF — a **conditional UPDATE guarded on the shown state** (optimistic concurrency; stale drag 409s and the board re-syncs, no lost-update). The human transition is a **distinct authority path from the agent claim: it does NOT acquire the fence/lease (§6.2)**; it is an operator override, audited with `initiated_by_user_id` (§6.5), RBAC-gated (contributor/maintainer write, viewer read-only §12.3), and leaves the agent's live claim intact (agent's next fenced write still obeys §6.3). Search + filters are server-side, tenancy-scoped query params on the same read; view-toggle is a per-user read preference (localStorage + `?view=`), never coord state.** | Rejected: a **stored `board_column` field** (dual source of truth with `state` — drift + a second write on every transition; the projection is free); **`blocked` as a 6th column** (an item would have to leave its workflow lane to be blocked — loses "where in the flow is this blocked thing", contradicts the validated 5-column mock); **DnD acquires the agent's fence/lease** (a human is not competing for the work-item claim — taking the lease would fight the agent and violate the per-item custody model §6.2/6.3; the human path is a separate authority); **last-writer-wins on the status write** (silent lost-update when an agent advances the item mid-drag — optimistic `expectedFrom` guard + 409 is the correct, cheap fix); **client-authored state change** (breaks the §13 BFF choke + no-P2P — every mutation is control-plane-mediated) |
| 038 | Responsive console — CSS-first single tree, desktop/tablet/mobile (Theme F, NFR-USE2/NFR-UI; CEO cross-cutting directive 2026-08-12, ISI-2327) | **One responsive app, one SSR tree, no separate mobile build and no device-class server routing. Layout keys off **viewport/container width via Tailwind breakpoints + CSS container queries** on the existing token system (§13) — reuse, no new UI framework, no device-detection lib. Canonical breakpoints (shared tokens, drive the UX kit AND the test matrix): mobile `<768` (top bar + 5-item bottom nav + slide-in drawer, works to 360px portrait), tablet `768–1024` (collapsible icon rail overlay, no bottom nav), desktop `>1024` (full labeled left rail). Navigation is the single adaptive shell — the three breakpoints are layout expressions of the ONE role-based adaptive-nav model (§13, screens 16/18): breakpoint collapse and RBAC filtering **compose**, no responsive-specific authz path. Per-screen = reflow of the same screen (Kanban→tabs+h-scroll, List→3-col+row-expand, Dashboard→stack, File tree→dropdown, Org diagrams→pinch-zoom+swimlane-stack, Search→full-width/icon, Settings→accordion), never a reduced feature set. Touch is a v1 bar: ≥44px targets, no hover-only affordances (tap/focus equivalent required), pull-to-refresh + pinch-to-zoom. Data path untouched — same BFF payloads, same RBAC wall, same one SSE bus; the browser (not the server) decides layout.** | Rejected: **UA-sniffing / device-class server routing** (unreliable, cache-hostile, splits the render tree + doubles the surface to test); a **separate mobile SPA or native app** (second codebase + second auth/SSE integration for the same read-mostly console — YAGNI at v1); a **"mobile-lite" reduced feature set** (contradicts the CEO "ALL screens" directive + fragments the RBAC/data contract — reflow keeps one contract); adding a **CSS/UI framework** for responsiveness (Tailwind already ships the breakpoint + container-query engine — ponytail); **viewport-conditioned API payloads** (a second data path + a new authz branch — the wall must stay single, §12.3) |
...
| 039 | Global cross-entity search (Theme P, FR-SEARCH*; CEO mock validation 2026-08-12, ISI-2323) — *§17.5* | **Postgres full-text search (`tsvector`/`tsquery` + GIN, `pg_trgm` for names) over the schemas KSquad already owns (`coord` work-items/comments/artifacts + build-snapshot *paths*, `scm` mirror, Run metadata) + the apiserver informer cache for CRD-backed entities (`Agent`/`Team`/`Role`/`Project`); a `pkg/search` **library in the apiserver** (reuse the trusted service + §12.3 RBAC middleware — no new binary), one `GET /api/v1/search` proxied by the BFF (§13). **RBAC scope applied *in the query*** — caller → `auth.project_memberships` → allowed `project_id` set → `WHERE project_id = ANY(...)` before rank/LIMIT, so out-of-scope entities never enter results/previews/**counts** (existence-hiding, §8.7d); admin = fleet bypass. Free text → **`websearch_to_tsquery`** (literal terms, injection-safe) + parameterized binds, so special chars never reach the engine as syntax and empty/no-match short-circuit to empty states. Derived read-model — no new store of record, no coordination path (R6). Dedicated search index (OpenSearch/ES) is the **named upgrade path** (OQ21 trigger), not built until measured need** | Dedicated search cluster at v1 (3rd stateful surface + reindex/dual-write lag + new tenancy/blast-radius surface — breaks S1 ≤4h, ADR-001; deferred as the upgrade path); a materialized `search` projection table (reindex pipeline + staleness for data already live in coord/scm/cache — Postgres FTS reads at query time); a separate search microservice (needless deployment; the RBAC choke point + coord DB already live in the apiserver); **post-filtering** a broad result set by RBAC in app code (leaks existence via counts/latency — the scope predicate must be *in* the query); building `tsquery` from raw user string (FTS-operator/SQL injection — `websearch_to_tsquery` + binds instead); searching **memory-record content** (violates the FR-E7/§7.3 untrusted-knowledge trust boundary) or raw file/blob bodies (build browser is the contents surface — Phase 2) |
| 040 | Coord audit table shape — audit-only, not a trace firehose (F1; Story 2.1 review ISI-2339 → ISI-2340, resolve-before-8.11) — *§6.5* | **The append-only Postgres coord audit table (`coord.audit_log`, née the Story-2.1 `run_event`) holds ONLY the low-volume, immutable COORDINATION audit — claim acquired/renewed/released, comment added, artifact registered, state transition, completion — each with `principal` + `initiated_by_user_id` + fence + timestamp on a clean **monotonic `bigserial`** sequence; append-only by trigger (UPDATE/DELETE/**TRUNCATE** rejected — F2), so **structurally immutable without qualification**. The **high-volume shim Run-trace firehose** (`tool_call\|llm_call\|build_output\|error`, §10.1) does **NOT** persist here — nor to any new Postgres table in v1 — it rides **SSE live (§10.1) + opt-in OTel export (§17.2)**, the "Run logs = coord audit + SSE, no new data path" model already pinned in §13. **Story 8.11 wires the shim trace to SSE + OTel emission, not a coord table** (so 8.11 needs no migration).** | Rejected: (a-literal) `audit_log` + a **time-partitioned Postgres `run_trace`** firehose table (a new stateful surface + partition/retention machinery to re-store telemetry OTel already owns, and §13 explicitly says "no new data path" — the working-tree draft's over-reach, corrected here); (b) keep **one `run_event` + declarative time-partitioning + `DROP PARTITION` retention** (partition-DROP **bypasses** the "structurally immutable" guarantee the table advertises, and interleaves trace noise into the monotonic audit `bigserial` → gaps under an `event_type` filter — the exact F1 defect); the original **single unified table** (unprunable firehose under the append-only trigger — the reported defect) |
| 041 | Credential seat-model / AUP boundary (Theme G, FR-G1/G2; ISI-2290 ratified CEO 2026-08-12, memo v3) — *§11.2* | **Two credential *classes* chosen by who the consumer is, not by runtime family: (A) a **human running their own squad** authenticates **BYO-per-human-seat** — one `CLAUDE_CODE_OAUTH_TOKEN` per human's *own* Pro/Max/Team subscription (§11.1 one-login-many-mounters is valid **only within that single seat's own work**); (B) any **shared / hosted / multi-agent / product** backend serving *other* people uses a **service account — Workload Identity Federation (keyless) or an org Anthropic API key under Commercial Terms**, never a consumer-seat token fanned across replicas serving different people. **Enterprises support BOTH paths (per-seat OAuth for humans + service-account/WIF/API for machines) — NOT API-only.** Human-seat path keeps a credential **health canary + rotation runbook** (carried from the retired ISI-2112 spike) so a lapsed seat is a legible `Paused(cred_expired)`. Non-binding on a specific negotiated Enterprise agreement (custom terms).** | Rejected: **pool one company Pro/Max subscription across a fleet of agents / many users** (Anthropic Consumer Terms §2 no-sharing + §3 no-automated-access-except-API — *not defensible*, ISI-2290 memo v3 TL;DR); **model the enterprise path as API-key-only** (contract supports both — over-restricts humans doing their own work); **embed a human-seat OAuth token in a shared platform backend** (making the account available to others — Consumer Terms §2); **treat §11.1 "many pods, one login" as unrestricted** (it is per-seat, single-owner — the boundary is *who the pods serve*, not pod count) |
---
...
| FR-B1…B5 (B1…B4 LOCKED; B5 = sub-tickets r24 + dual-view r25) | §6 coordination record (Postgres, fenced); §6.1 `work_item.parent_id` adjacency + §13 Tickets tree; §13 dual-view board-derivation + DnD status-transition API (ADR-037); §8.6 `blocked`-as-condition |
| FR-F1 (Tickets dual-view Kanban+List; ISI-2326) | §13 Project Tickets dual view — board = `state` projection, DnD human status-transition, view-toggle persistence, search/filters (ADR-037) |
...
| FR-SEARCH1…5 + NFR-PERF3 (global search, r9) | §17.5 `pkg/search` Postgres FTS over coord/scm/CRD-cache, RBAC-in-query (allowed-`project_id` predicate, existence-hiding §8.7d), injection-safe `websearch_to_tsquery`; §13 BFF proxy + top-bar surface; OQ21 relevance/index-freshness (Architecture-owned); ADR-039 |
...
| Responsive console — desktop/tablet/mobile ALL screens (Theme F, NFR-UI; CEO cross-cutting directive 2026-08-12, ISI-2327) | §13.1 responsive layout strategy — CSS-first single SSR tree (Tailwind breakpoints + container queries), canonical breakpoints (mobile `<768` bottom-nav+drawer / tablet `768–1024` icon rail / desktop `>1024` full rail), navigation = single adaptive shell composing with §12.3 RBAC filtering (no responsive authz path), per-screen reflow table, ≥44px touch targets + no-hover-only + pull-to-refresh + pinch-zoom, data path untouched (same BFF/RBAC/SSE); ADR-038; cross-cutting → PRD NFR-UI, Epic 8 per-screen mobile+tablet ACs, testing viewport matrix (360/768/1024/1440), observability viewport/device-class RUM dimension. No locked decision reopened |
