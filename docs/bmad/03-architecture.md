...
- **Capability flags are first-class (FR-D4, R3):** streaming / tool-calls / interactive-prompt /
  credential-type are negotiated on the Agent Card; the core treats gaps as declared capabilities,
  never as special-cased hacks. A runtime with no interactive-prompt support advertises that; the core
  routes around it.
...
*Satisfies:* FR-D1…D5, NFR-EXT1/EXT2, R3, R11. *Spike-gated:* reference shim + conformance assertions
(ISI-2114), pinned A2A/MCP rev. *Trade recorded:* ADR-008 (sidecar shim), ADR-009 (pinned adapter
seam).
