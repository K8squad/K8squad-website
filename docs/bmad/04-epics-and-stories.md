...
| 8.6 | As an operator, I want to **surface credential/auth state**, including a clear paused-on-expiry signal. | **Given** an Agent whose Run is `Paused` on an expired token, **When** I view credential state, **Then** the console shows a clear paused-on-expiry signal (supports S10, Epic 7.4). | UX `05-credential-auth-state`. FR-F6. **[CEO 2026-08-12]** Adds a **"Connect Claude"** one-click browser-OAuth button (+ `ksquad auth login` CLI parity, 7.7) and **per-agent credential health** — *connected / refreshing / expired* — so the user sees status but never handles token strings; the *expired* state offers **one-click re-login**. This is the Settings → Credentials surface in the nav IA (8.13). |
