...
and surfaces credential/auth state. Uses the BFF to proxy SSE and hide the Go API. The screens sit
inside a **Project-rooted navigation shell** (story 8.13, CEO 2026-08-12): Dashboard/Overview are
global; Agents, and the Project sub-tree (Build/Tickets/Runs/Discussion), scope to a selected Project —
the IA mirrors the `project_id`-scoped data model (§6.1).
...
| 8.13 | **[CEO 2026-08-12]** As an operator, I want a **navigation shell with a Project-rooted hierarchy** so the console is a contextual tree, not a flat screen list. | **Given** the console, **When** I use the nav rail, **Then** it presents: **Dashboard** (fleet-global, 8.8) and **Overview** (squad-global, 8.1) as **global** entries; **Agents** grouping the org diagram (8.10) + agent detail (8.11), **filterable by squad/project**; **Project** as the **root context** — a Project selector/breadcrumb whose sub-navigation is **Build (8.7) · Tickets (work items) · Runs (8.2) · Discussion (8.10 room)**, all **scoped to the selected Project**; and **Settings** grouping **Configuration (8.12)** + **Credentials (8.6)**. **And** switching Project re-scopes all Project sub-views; **And** the 12 existing screens are unchanged **content** — only the nav/breadcrumb shell that wraps them is new. | UX: nav rail + breadcrumb + Project sub-nav (ISI-2150, designer). **Architecture-context: Project is the scoping root because it already is in the data model** — work items, Runs, discussion, and workspace/builds are all `project_id`-scoped in the coordination record (§6.1) under the `Project` CRD (§5.1). The IA mirrors the tenancy/scoping model; Dashboard/Overview are cross-Project. Nav-shell/read-composition only — no new backend, scope-guarded (R6, not an IDE). |
| 8.13 | **[CEO 2026-08-12]** As an operator, I want the console organized as a **Project-rooted navigation hierarchy** (not a flat screen list) so Builds, Tickets, Runs, and Discussions are always navigated in the context of a selected Project. | **Given** the console shell, **When** it loads, **Then** the left nav rail renders the hierarchy — **Dashboard** (fleet: health/consumption/live mapping, 8.8) · **Overview** (squad status/activity, 8.1) · **Agents** (org diagram 8.10 + agent detail 8.11, **filterable by squad/project**) · **Project** (root context) · **Settings** → {**Configuration** 8.12, **Credentials** 8.6}; **And** a **Project context selector** (top-of-rail + breadcrumb) sets the active Project, whose node expands to sub-navigation **Build (8.7), Tickets (8.14), Runs (8.2 + run history), Discussion (10.3)** — all **scoped to the selected Project**; **And** **Dashboard + Overview stay global** (not project-scoped); **And** a **breadcrumb** always shows `Project › section`; **And** existing screen content (8.1–8.12) is **unchanged** — only the nav wrapping/routing changes (App Router **nested layouts/routes**). | **[IA/nav only — no new backend]** Realizes the CEO 2026-08-12 IA. New screens: **Project context selector** + **Project sub-nav** (Build/Tickets/Runs/Discussion). Nav-rail + breadcrumb mock owned by **Graphic Designer** (new design ticket). NFR-USE2. Scope guard (R6): routing, not a new data surface. |
...
page 8.12 (12th console screen, ISI-2150) — opt-in, per-signal routing, extends FR-A/NFR-OBS**. **No
