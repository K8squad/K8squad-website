...
| 2.11 | **[CEO 2026-08-12]** As the system, I want a **durable scheduled-resume timer** for a Paused(rate_limited) Run so it wakes **exactly when the window clears — a single timer, not a poll loop, zero wasted API calls**. | **Given** a Run Paused(rate_limited) (3.7) with **no fallback** configured, **When** the pause is recorded, **Then** `resume_at = now + Retry-After` is **persisted in the coordination record** and the control plane **schedules one wake/heartbeat at `resume_at`** (no periodic polling); **When** the timer fires, **Then** the Run auto-resumes (3.7). **And** if `Retry-After` is absent, `resume_at` uses **exponential backoff with jitter**. **And** the timer is **durable** — re-armed from `resume_at` after a controller restart (no lost/duplicated wake). | Arch §6 (durable `resume_at` in coord record) + §8 (reconciler arms/honors it). The CEO **no-fallback path**: one scheduled wake, not a poll loop. **Impl: ISI-2296** (auto-resume heartbeat). Depends 3.7; fallback path = 5.11. |
