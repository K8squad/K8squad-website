...
| 7.6 | **[CEO 2026-08-12]** As the system, I want to **detect LLM provider rate limits and auto-pause/resume** the Run — with per-credential attribution and squad-level re-routing — so a `429` never fails a Run opaquely or silently burns budget. | **Given** a Running Run whose provider returns a rate-limit response (e.g. HTTP 429 + `Retry-After`), **When** the shim detects it, **Then** the shim surfaces a **standardized `rate_limited` signal over A2A** (§7.1, capability/signal metadata 5.2) carrying the `Retry-After` window, and the Run moves to **`Paused(rate_limited)`** with `Retry-After` tracked on `Run.status` (§5.2/§5.3), **auto-resuming when the window clears** — never an opaque failure. **And** repeated rate limits apply **bounded exponential backoff** before resume. **And** the pause is **attributed to the specific BYO credential/principal** that hit the limit (per-credential, §10 / 7.1) — not the platform. **And** a squad **coordinator (2.9) may re-dispatch** the blocked work item to another agent whose credential/endpoint is not rate-limited — via the coordination record (open-item → fenced claim), **never P2P**. **And** the event is emitted to the **NATS seam (Epic 12)**, surfaced on the **dashboard (8.8)** and **Run logs/trace (Epic 13)** as `ksquad.run.paused{cause=rate_limited}` + a `Retry-After` gauge. | **[CEO 2026-08-12]** Impl **ISI-2297** (detection + pause/resume) + **ISI-2296** (auto-resume heartbeat). Extends 7.4's pause/resume with a new `rate_limited` cause (parallels cred-expiry). Standardized signal keeps it **runtime-neutral, not Claude-shaped** (§7.2). Re-routing rides the locked shared-work-item + fencing model (FR-B3, no P2P). Impacts §8 state machine (Epic 3), §7/§10 shim (Epic 5.2), Epic 13 obs (`cause=rate_limited`), Epic 12 seam. |
| 7.6 | **[CEO 2026-08-12]** As an operator, I want **rate-limit pause/resume attributed per credential** (BYO-subscription), so the platform knows *which* credential is throttled and can route around it. | **Given** a Run Paused(rate_limited) (3.7) triggered by the shim signal (5.10), **When** the platform records it, **Then** the pause is **attributed to the specific credential/principal** whose subscription was throttled (first-class under the BYO-subscription lock, §11), with `Retry-After` tracked **per credential**; **And** this drives coordinator **re-routing to an Agent with a *different* credential (2.10)**; **And** the pause is a distinct **reason** in the same pause/resume family as credential-expiry (7.4) — the operator sees "paused: rate-limited (credential X, resumes ~T)" on the console (8.6/8.11). | Arch §11 (per-user Secret ref → per-credential attribution by construction), §7.2 (credentialLifecycle metadata gains a rate-limit dimension). Reuses 7.4 pause/resume machinery; consumption/attribution ties to dashboard 8.8 + Epic 13. |
