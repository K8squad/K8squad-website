...
| 8.14 | **[CEO 2026-08-12]** As an operator, I want a **Project → Tickets view** — the work items scoped to the selected Project — so I can see and open tickets without leaving the console. | **Given** a selected Project, **When** I open **Tickets**, **Then** I see its **work items** with **status, assignee, and counts** (open/claimed/blocked/done) from the **Epic 2 coordination record**; **And** opening a ticket shows its **comments (append-only, provenanced), artifacts, and checkout/holder** state; **And** the view is **read + navigate** (CRD compose/edit stays 8.5; claim/coordination stays server-side, R6) and links each ticket to its **Runs (8.2)** and **build outputs (8.7)**. | Fills the one Project sub-item without an existing screen (Build=8.7, Runs=8.2, Discussion=10.3 already exist). Reads the coordination DB (Epic 2, §6.1) via apiserver; SCM-synced tickets (Epic 11) show provenance badges. New content screen — mock owned by the Designer nav/IA ticket. |
...
another credential, fenced) + 13.7 (reason-labeled pause alert) — extends FR-A/FR-D/FR-G/NFR-OBS**. **No
...
| 9 | **Console screens** (12+ screens, dark+light, v2 logo, **Project-rooted nav IA**) | E8 | 8.1–8.14 (theming 8.9, org diagram 8.10, agent detail 8.11, settings/OTelConfig 8.12, **nav shell 8.13, Project Tickets 8.14**) |
