...
Mutation-proof harness:
  ...
  --mutate=HARD_FAIL_DEGRADE hard-fail when liveRuns source is unavailable (5xx / throw)    -> C1 RED (also C4)

Baseline exits 0; each --mutate exits 1.
"""
import sys

# 8.8a liveRuns sub-payload — runs active, one is Paused(rate_limited) with a real resume_at.
SCENARIO_LIVE = {
    "available": True,
    "data": [
...
def evaluate(design, mut):
    ...
    # C4 — rate-limit indicators degrade gracefully (verified via C2 transport and C1 availability above).
    # The rate-limit/fallback figures themselves come from 13.9; this panel SSE-consumes them.
...
def main(argv):
    ...
    if mut is None:
        ...
        print("[model] §8.8f conformant Live Runs panel  : 0 violation(s); "
              "sse-existing-bus, read+navigate-only, coord-record-resume_at, "
              "real-rows-only, no-second-authz, bounded-cardinality")
        print("[runs] PASS")
        return 0
